...
create table member (
    ...
    constraint ck_member_role check (member_role in('U', 'A')) -- Member(회원 계정), Admin(관리자 계정)
);
...
comment on column member.address_sub is '상세주소';
